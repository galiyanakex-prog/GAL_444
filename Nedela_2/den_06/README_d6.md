...
- Session Memory: при старте лог не загружается в контекст автоматически (сессия всегда новая), но история поднимается вручную командой `/load_history`; ручное сохранение — `/save` (дозапись без дублей);
- Команды: `/history` (пометки `[в окне]`/`[вытеснено]`), `/window`, `/save`, `/load_history`, `/clear`, `/exit`.
...
Один класс `Agent` со стратегиями внутри (диспетчеризация через if/elif — учебный аналог Strategy pattern):
...
- **Редактирование ввода:** подключён модуль `readline` (стандартная библиотека, Linux) — стрелки перемещают курсор и листают историю ввода, Home/End/Delete работают; на платформах без `readline` (например, Windows) ввод работает без редактирования;
- **Кодировка ввода:** stdin настраивается на UTF-8 с `errors='replace'` — ввод в другой кодировке (например, CP1251 из ярлыка/терминала) не прерывает программу: «битые» байты заменяются символом-заглушкой;
- Стиль: учебный однофайловый скрипт, комментарий почти к каждой строке; логика агента инкапсулирована в класс `Agent` (CLI-цикл работает с агентом только через публичные методы), внешние зависимости — только `requests` и `python-dotenv`.
...
- Логика агента инкапсулирована в класс `Agent` (требование задания Дня 6: «агент должен быть отдельной сущностью (класс)»);
...
- Журналирование непредвиденных ошибок в отдельный `den_6_error_*.log` для каждого варианта с защитой от «молчаливог
о» закрытия окна терминала.

## Ручная проверка

Пошаговый сценарий ручной проверки проекта (чек-лист приёмки с отметками) — в файле
[`Проверка_Д6.md`](Проверка_Д6.md). Включает подготовку, проверку всех функций дня
(три стратегии контекста, три типа памяти, отдельные файлы вариантов), устойчивость
