# ============================================================================
# День 6 — den_6_Kod.py (все три техники через CLI-флаги, класс Agent)
#
# КЛЮЧЕВАЯ ИДЕЯ:
# Три варианта A/B/C — это три стратегии поверх одного и того же каркаса
# «чат-бот с историей». Вместо трёх копий кода делаем один класс Agent,
# внутри которого выбор стратегии — два флага: --context (как формировать
# окно запроса) и --memory (что и как сохранять/загружать). Это первый шаг
# к пониманию, почему в больших проектах стратегии оформляют отдельными
# модулями (Strategy pattern): сейчас стратегии — методы одного класса
# с диспетчеризацией по строке режима, позже их можно вынести в отдельные
# классы-стратегии.
#
# Запуск:
...
class Agent:
    ...
    def __init__(self, system_prompt: str, context_strategy: str = "sliding_window",
        ...
        # сообщения и не затирал то, что уже дописано append_log().
        self.saved_count = 0
    ...
    # Счётчик saved_count нужен, чтобы /save не затирал live-лог (раньше файл
    # открывался на «w» и терялись строки, дописанные append_log() во время
    # живого диалога) и чтобы повторные /save не создавали дублей.
    ...
    def load_history(self, filepath: str):
        # Для session-памяти лог поднимается в контекст только вручную (команда
        # /load_history): при старте сессия всегда начинается с чистого листа.
        if self.memory_type == "session":
            # Список, в который восстанавливаем сообщения из файла.
            loaded = []

            # Если файла нет — контекст остаётся пустым (загружать нечего).
            if os.path.exists(filepath):
                # Читаем Markdown-лог построчно (кодировка UTF-8).
                with open(filepath, "r", encoding="utf-8") as f:
                    ...
                    for line in f:
                                ...
                                loaded.append(
                                    {
                                        # Роль сообщения — пользователь.
                                        "role": "user",
                                        # Текст сообщения из лога.
                                        "content": text,
                                        # Время загрузки (время реплики в логе не хранится).
                                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                    }
                                )
                        ...
                        # Строка реплики агента: «**Агент:** текст».
                            ...
                            # Достаём текст после префикса и убираем лишние пробелы.
                            text = stripped[len("**Агент:**"):].strip()

                            # Пустые реплики в историю не добавляем.
                            if text:
                                # Пропускаем повторяющиеся приветствия: каждый запуск
                                # дописывает в лог одинаковое приветствие, и без этой
                                # проверки /load_history тащит все их в контекст.
            ...
            # Все загруженные сообщения уже лежат в файле — помечаем их
            # сохранёнными, чтобы последующие /save и /exit не записали их повторно.
            self.saved_count = len(self.full_history)

            # Считаем заново, сколько сообщений стоит за пределами окна.
            self.evicted_total = max(0, len(self.full_history) - self.window)

            # Возвращаем количество восстановленных сообщений (для вывода в CLI).
...
# Загружаем память прошлой сессии (поведение зависит от memory_type).
if args.memory == "session":
    # Session-память начинается с чистого листа: лог в контекст не поднимаем,
    # но если прошлый лог есть — подсказываем, как загрузить его вручную.
    if os.path.exists(LOG_FILE):
        # Уведомление с явной подсказкой про ручную загрузку.
        print(
            f"[Память] Найден прошлый лог: {LOG_FILE} "
            "(в контекст не загружается — сессия новая, /load_history — загрузить)"
        )
# Для compressed и layered память поднимается автоматически (саммари и слои).
else:
    # Загрузку выполняет агент: поведение зависит от memory_type.
...
while True:
    try:
        ...
        # (например «/save») уходила в LLM как обычный текст: модель отвечала
        # прозой, а сама команда попадала в лог как реплика пользователя.
        ...
        # Доступна во всех режимах: дозаписывает только ещё не записанные сообщения.
        if user_input == "/save":
            # Сохранение делает агент: файл открывается на дозапись, live-лог не затирается.
        ...
        # Обрабатываем команду /load_history — вручную поднять лог в контекст.
        if user_input == "/load_history":
            # Для compressed и layered память поднимается сама при старте
            # (саммари из файла саммари, слои — из лога), вручную это не нужно.
            if args.memory in ("compressed", "layered"):
                # Объясняем, что ручная загрузка в этих режимах не применяется.
                print(
                    f"[Память] В режиме memory={args.memory} память загружается "
                    "автоматически при старте\n"
                )
                # Переходим к следующей итерации цикла.
                continue

            # Session-память: грузим прошлый лог в контекст по запросу.
            if not os.path.exists(LOG_FILE):
                # Файла нет — сообщаем и продолжаем работу (не падаем).
                print(f"[Память] Файл не найден: {LOG_FILE}\n")
                # Переходим к следующей итерации цикла.
                continue

            # Загрузка заменит текущую историю, поэтому спрашиваем подтверждение,
            # только если в контексте уже что-то есть (нечего терять — грузим молча).
            if agent.get_history():
                # Спрашиваем согласие на перезапись текущей контекста.
                answer = input("[Память] Текущая история будет заменена логом. Продолжить? (y/n): ").strip().lower()

                # Любая буква, кроме «y»/«yes», означает отказ.
                if answer not in ("y", "yes", "д", "да"):
                    # Отказ уважаем: контекст остаётся как был.
                    print("[Память] Загрузка отменена\n")
                    # Переходим к следующей итерации цикла.
                    continue

            # Загрузку и разбор Markdown-лога выполняет агент.
            loaded_total = agent.load_history(LOG_FILE)

            # Сколько из загруженных сообщений реально помещается в окно контекста.
