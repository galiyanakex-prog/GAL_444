# ============================================================================
# День 9 — den_9_Kod.py (управление контекстом: сжатие истории, класс Agent)
#
# КЛЮЧЕВАЯ ИДЕЯ:
# Модель stateless: каждый обмен отправляет ей ВЕСЬ диалог, поэтому запрос
# растёт как 1+2+3+…+N и упирается в окно (262K токенов). День 9 ломает этот
# рост: последние N сообщений едут в запрос «как есть» (оперативное окно),
# а всё, что старше, сворачивается в саммари — и дальше в запрос идёт не
# полная история, а короткое саммари + окно. Саммари хранится ОТДЕЛЬНО (поле
# self.summary + файл *.summary.md) и подставляется «под капотом» вторым
# system-сообщением, а не копипастом в текст пользователя.
# Три требования задания и где они лежат в коде:
#   1) «храните последние N сообщений как есть»  → self.window (--keep/--window);
#   2) «остальное заменяйте summary (каждые 10)» → автосжатие по порогу
#      COMPRESS_EVERY (флаг --compress-every, по умолчанию 10 — ровно из
#      формулировки задания);
#   3) «summary отдельно и вместо полной истории» → self.summary, SUMMARY_FILE,
#      _build_messages_compression (system + саммари + окно).
# Сравнение «до/после» — команда /compare: один контрольный вопрос про ранний
# контекст задаётся в двух режимах (без сжатия / со сжатием), печатаются оба
# ответа (качество) и токены с рублями (расход). ВАЖНО (вывод из чата курса):
# экономия токенов ≠ экономия рублей — сама суммаризация это отдельный платный
# запрос (а исходящие втрое дороже входящих: 33 против 11 ₽/1M) и она сбрасывает
# prompt-cache, меняя префикс запроса. Поэтому /compare печатает экономию
# ДВУМЯ строками: без учёта суммаризации и с её учётом + оговорку про кэш.
# Донор (предыдущий день) делал токены ВИДИМЫМИ; этот день делает их МЕНЬШЕ,
# пользуясь тем же счётчиком как мерой эффекта (Суть_N2.txt §2.2 «History
# Compression»: сжатие создаёт иллюзию непрерывности, но теряет детали).
#
# Запуск:
#   python den_09/den_9_Kod.py --context compression --memory compressed
#   python den_09/den_9_Kod.py --context compression --compress-every 10
#   python den_09/den_9_Kod.py --context compression --summary-word-limit 50
#   python den_09/den_9_Kod.py --context sliding-window --memory session  # «до»
# Внутри чата: /compare (сравнение до/после), /compress on|off (сжатие на лету).
# ============================================================================
...
# Импортируем модуль json для сохранения и загрузки контекста агента (День 9).
import json
...
parser = argparse.ArgumentParser(
    description="Чат-бот Дня 9: сжатие истории для экономии токенов (окно «как есть» + "
    "саммари вместо старой части диалога), сравнение качества и расхода до/после."
)
...
# Флаг --log: имя лог-файла (по умолчанию Den_9_log.md).
parser.add_argument(
    "--log",
    type=str,
    default="Den_9_log.md",
    help="Имя лог-файла.",
)

# Флаг --context-file: путь к JSON-файлу контекста (День 9).
parser.add_argument(
    "--context-file",
    type=str,
    default="den_9_context.json",
    help="JSON-файл контекста для сохранения/загрузки (по умолчанию den_9_context.json).",
)

# Флаг --fresh: начать новую сессию, игнорируя прошлый JSON-снимок (День 9).
...
# Флаг --max-context-tokens: искусственный лимит контекстного окна (День 9).
...
# Флаг --max-tokens: верхний предел длины ответа модели (День 9).
...
# Флаг --token-log: имя CSV-журнала роста токенов и стоимости (День 9).
parser.add_argument(
    "--token-log",
    type=str,
    default="den_9_tokens.csv",
    help="CSV-журнал токенов и стоимости по каждому обмену (по умолчанию den_9_tokens.csv).",
)

# Флаг --price-in: цена миллиона входящих токенов в рублях (День 9).
parser.add_argument(
    "--price-in",
    type=float,
    default=None,
...
# Флаг --price-out: цена миллиона исходящих токенов в рублях (День 9).
parser.add_argument(
    "--price-out",
    type=float,
    default=None,
...
# --- Флаги Дня 9: настройки сжатия истории -----------------------------------
...
# В доноре порог был жёсткой константой (8); День 9 делает его настраиваемым,
...
# Путь к JSON-файлу контекста (День 9): относительный путь фиксируется за
...
# Путь к CSV-журналу токенов (День 9): та же схема — относительный путь
...
# запускаем сжатие. В доноре это была жёсткая константа (8); День 9 делает порог
...
# День 9 (Ч6, Ч7): промпт стал ШАБЛОНОМ — в него подставляются язык переписки
...
ERROR_LOG = os.path.join(BASE_DIR, "den_9_error.log")

# Версия формата JSON-снимка контекста (День 9): при изменении структуры
...
# В версии 3 добавлены настройки сжатия Дня 9: порог --compress-every, лимит слов
...
# Признаки того, что API сам отказал из-за переполнения контекста (День 9).
...
# --- Валидация и эффективные значений флагов сжатия Дня 9 ---------------------
...
# Функция дозаписи строки обмена в CSV-журнал токенов (День 9).
...
# 5. Токен-функции: локальная оценка токенов (День 9) ---------------------------
...
class Agent:
    ...
    def __init__(self, system_prompt: str, context_strategy: str = "sliding_window",
        ...
        max_context_tokens — лимит контекста для проверки перед отправкой (День 9);
        max_tokens — лимит длины ответа модели (День 9);
        price_in_per_m / price_out_per_m — цены 1M токенов в рублях (День 9);
        compress_every — порог автосжатия в сообщениях (День 9, по умолчанию 10);
        summary_word_limit — лимит длины саммари в словах (День 9);
        compress_temperature — температура запроса сжатия (День 9)."""
        ...
        # --- День 9: настройки сжатия истории --------------------------------
        ...
        # День 9 (Ч8): отдельный накопитель токенов СЛУЖЕБНЫХ запросов сжатия.
    ...
    # Внутренний метод: учёт блока usage из ответа API (День 9).
    ...
    def _record_usage(self, usage, is_main=True):
        ...
        # День 9 (Ч8): служебный запрос (сжатие) копим ОТДЕЛЬНО — для строки
        # экономики /compare «с учётом суммаризации». Это подмножество usage_total.
        else:
    ...
    # Публичный метод (День 9): токены ПОСЛЕДНЕГО запроса — то есть «токены текущего
    ...
    # Публичный метод (День 9): токены ВСЕЙ ИСТОРИИ диалога — накопительная сумма
    ...
    # Публичный метод (День 9): токены ПОСЛЕДНЕГО ответа модели. None — если usage
    ...
    # Публичный метод (День 9): стоимость последнего обмена в рублях по тарифу
    ...
    # Публичный метод (День 9): стоимость всей сессии в рублях — «прайс» накопленного
    ...
    # Публичный метод (День 9, Ч8): стоимость ВСЕХ служебных запросов сжатия за сессию.
    ...
    def _ask_llm(self, messages, is_main=True, temperature=None):
        ...
        # День 9: если задан --max-tokens, передаём его API — это верхний предел
        # генерации, самый дешёвый способ урезать дорогие исходящие токены.
        if self.max_tokens is not None:
            # Кладём лимит длины ответа в тело запроса.
            data["max_tokens"] = self.max_tokens

        # День 9 (Ч6): температура передаётся ТОЛЬКО для служебного запроса сжатия
        ...
        for attempt in range(len(RETRY_DELAYS) + 1):
            ...
            try:
                ...
                # День 9: считываем блок usage ДО проверки choices — токены за этот
            ...
            # Перехватываем ошибку HTTP (кроме уже обработанной 429).
                ...
                # День 9: локальная оценка приблизительная, поэтому лимит не гарантирует
    ...
    # Публичный метод (День 9): собирает ДВА варианта запроса для /compare.
    ...
    # Публичный метод (День 9): локальная оценка экономии для /compare (без API).
    ...
    # Публичный метод (День 9): ОДИН запрос к LLM по готовому списку сообщений
    ...
    # Внутреннее (День 9, Ч6): определяет язык переписки по старым сообщениям.
    ...
    def _compress_history(self, old_messages, summary, prompt=None):
        ...
        # День 9 (Ч6): определяем язык переписки, чтобы саммари было на нём,
        # а не на языке промпта (иначе саммари «съедает» смысл для ответов).
        language = self._detect_language(old_messages)

        # День 9 (Ч6, Ч7): заполняем шаблон промпта языком и лимитом слов.
        # Для compressed-памяти берём общий COMPRESS_PROMPT, для layered —
        # послойный шаблон из COMPRESS_PROMPTS. Лимит в словах формулируется
        # именно в промпте — так он ограничивает текст, а не обрезает ответ.
    ...
    # Внутренний метод (День 9): проверка запроса на переполнение контекста ДО отправки.
    ...
    def send_message(self, user_message: str):
        ...
        # День 9: проверяем переполнение ДО обращения к API. Реплику пользователя
        ...
        # День 9: обмен состоялся — увеличиваем счётчик обменов. Он нужен как номер
        # строки в CSV-журнале и как множитель в прогнозе стоимости.
        self.exchanges += 1

        # День 9: дозаписываем строку обмена в CSV-журнал токенов. Ошибка записи
    ...
    def save_summary(self, filepath: str):
        ...
        if self.memory_type == "layered":
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                ...
                f.write("# Саммари диалога — День 9 (послойно)\n")
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            # Пишем заголовок первого уровня с названием техники.
            f.write("# Саммари диалога — День 9\n")
            # Пишем строку с датой и временем последнего обновления.
            f.write(f"Обновлено: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    ...
    # --- День 9: JSON-персистентность контекста ------------------------------

    # Сохранение полного состояния агента в JSON-файл (ядро Дня 9).
    ...
    def save_context_json(self, filepath: str):
        ...
        snapshot = {
            ...
            # Послойные саммари (v3): {слой: текст}. Для layered — источник
    ...
    # Загрузка состояния агента из JSON-файла (ядро Дня 9).
    ...
    def load_context_json(self, filepath: str, cli_strategy: str = None,
        ...
        # поднимаем (структура неизвестна); v1/v2 мигрируем в v3 ниже, v4+ — отказ.
        ...
        try:
            ...
            # Восстанавливаем послойные саммари (v3). Для снимков v1/v2 ключа
            # нет — мигрируем: прежнее единое саммари было смешанным, относим его
            # в low (самый «терпимый» к потерям слой), high/mid остаются пустыми.
            snapshot_sbl = snapshot.get("summary_by_layer")
            if snapshot_version < 3 or not isinstance(snapshot_sbl, dict):
                # Свежий словарь пустых послойных саммари.
                self.summary_by_layer = {layer: "" for layer in LAYER_ORDER}
                # Единое саммари v1/v2 переезжает в low-слой (при его наличии).
                if snapshot_sbl is None and self.memory_type == "layered":
                    self.summary_by_layer["low"] = self.summary
                    # Для layered-памяти уведомляем о миграции снимка.
                    print("[Память] Старый снимок мигрирован в v3 (саммари перенесено в слой low)")
            else:
                # v3: поднимаем послойные саммари, дополняя недостающие слои пустым.
            ...
            # День 9: восстанавливаем накопительный токен-учёт. Снимки версии 1
            ...
            # День 9 (v3, Ч8): восстанавливаем накопитель токенов суммаризации.
            ...
            # День 9 (v3): восстанавливаем настройки сжатия из снимка, но НЕ затираем
            ...
            # недостающие поля подставляются дефолтами, диалог продолжается.
            if snapshot_version < CONTEXT_VERSION:
                # Сообщение не критичное: диалог продолжается.
                print(
        ...
        # Структура файла не соответствует ожидаемой (нет ключей, не те типы).
            ...
            self.evicted_total = 0
            # Токен-учёт сбрасываем, чтобы «залипшие» старые токены не выдавали
            # себя за стоимость текущей (битой) сессии.
            self.usage_total = {
                "prompt_tokens": 0,
                "completion_tokens": 0,
                "total_tokens": 0,
...
def cmd_help(agent, args):
    ...
    # /compare — для compressed и layered (ядро Дня 9).
    if args.memory in ("compressed", "layered"):
        print("  /compare — качество и расход до/после сжатия (compressed/layered)")
    else:
        print("  /compare — недоступна в режиме memory=session")
...
# Обработчик /compress on|off (День 9, Ч5): включает/выключает сжатие НА ЛЕТУ.
...
# Обработчик /compare (День 9, ядро дня): качество и расход «до/после» сжатия.
...
# Обработчик /save: ручное сохранение контекста в JSON (День 9).
...
# Обработчик /tokens: три счётчика токенов + локальная оценка (День 9).
...
# Обработчик /scenario: сравнение трёх сценариев расхода токенов (День 9).
...
# Обработчик /cost: стоимость обмена, сессии и прогноз на 100 обменов (День 9).
def cmd_cost(agent):
...
# Обработчик /load: ручная загрузка контекста из JSON (День 9).
...
if not os.path.exists(LOG_FILE):
    ...
    try:
        ...
        with open(LOG_FILE, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"# Лог диалога — День 9 (контекст={args.context}, память={args.memory}, окно={WINDOW})\n\n"
            )
...
agent = Agent(
    ...
    # День 9: настройки сжатия (None = «флаг не задан», возьмётся константа по умолчанию).
...
# --- День 9: автозагрузка контекста из JSON (до приветствия) ---------------
...
print(f"🤖 Чат-бот — День 9 (контекст={args.context}, память={args.memory}, окно={WINDOW})")
print(f"Лог: {LOG_FILE}")
print(f"Контекст (JSON): {CONTEXT_FILE}")
print("Команды: /history /window /summary /compress [on|off] /compare /layers /layer "
      "/save /load /tokens /cost /scenario /clear /help /exit")
...
while True:
    try:
        ...
        if user_input.startswith("/"):
            # Команда — первое слово строки (у /layer и /compress есть аргументы).
        ...
        # Обрабатываем /compress on|off (День 9, Ч5) — переключатель сжатия на лету.
        ...
        # Обрабатываем команду /compare (День 9, ядро дня) — качество и расход до/после.
        ...
        # Обрабатываем команду /save — ручное сохранение контекста в JSON (День 9).
        if user_input == "/save":
        ...
        # Обрабатываем команду /tokens — три счётчика токенов (День 9).
        ...
        # Обрабатываем команду /cost — стоимость обмена и сессии (День 9).
        if user_input == "/cost":
        ...
        # Обрабатываем команду /scenario — три сценария расхода токенов (День 9).
        ...
        # Обрабатываем команду /load — ручная загрузка контекста из JSON (День 9).
        if user_input == "/load":
        ...
        if user_input == "/exit":
            ...
            # День 9: сохраняем полный контекст в JSON перед выходом.
            agent.save_context_json(CONTEXT_FILE)
    ...
    # Перехватываем Ctrl+D (EOF) — корректный выход без ошибки.
        ...
        # День 9: сохраняем контекст и при выходе по EOF — агент «не выключался».
        agent.save_context_json(CONTEXT_FILE)
    ...
    # Перехватываем Ctrl+C — корректный выход без traceback.
        ...
        # День 9: сохраняем контекст и при прерывании — агент «не выключался».
        agent.save_context_json(CONTEXT_FILE)
        # Сообщаем о сохранении контекста.
