# run_log.md — сводный журнал прогонов проекта den_11
...
## Переменные проекта (§2.1 START-PROMT_d11.md)

| Переменная | Значение (вычислено на хосте) |
|---|---|
| `AI9_ROOT` | /home/u/Документы/111111/Обучение_Курсы/Основной_репозиторий/AI_9 |
| `KOD` | /home/u/Документы/111111/Обучение_Курсы/Основной_репозиторий/AI_9/Nedela_3/den_11 |
| `DEV` | $KOD/den_11_dev |
| `MP` | $DEV/meta_promt |
...
- Справочные файлы — все на месте: Step-3.5-Flash_params.md, МЕТА-ПРОМТ_den_N.md, DONOR,
  Суть_N3.md, Задание_Д11.txt, arch_den_11.md, arch_den_11_plan_2.md.
...
```bash
cd "$KOD"
koda --yolo -p "$MP/ПРОМТ_den11_<часть>.md"
```
...
| 2 | Хранилище | `$PY $TST/unit_runner.py test_storage` — сверка с ПРОМТ_den11_storage.md | 0 | ✅ |
| 3 | Память | `$PY $TST/unit_runner.py test_memory` — сверка с ПРОМТ_den11_memory.md | 0 | ✅ |
| 4 | LLM-клиент | `env -u API_KEY $PY $TST/unit_runner.py test_llm` — заглушка requests, без сети | 0 | ✅ |
| 5 | PromptBuilder | `$PY $TST/unit_runner.py test_prompt` — сверка с ПРОМТ_den11_prompt.md | 0 | ✅ |
| 6 | Агент | `$PY $TST/unit_runner.py test_agent` — сверка с ПРОМТ_den11_agent.md | 0 | ✅ |
| 7 | State machine | `$PY $TST/unit_runner.py test_state` — сверка с ПРОМТ_den11_state.md | 0 | ✅ |
| 8 | CLI | L1 `py_compile` всей сборки; сверка с ПРОМТ_den11_cli.md | 0 | ✅ |
| 9 | Отладка | L1–L4: unit_runner/smoke/scenario все exit 0 | 0 | ✅ |
| 10 | Финал | README/run.sh/run.desktop/Проверка_Д11/final_report; `API_KEY=test-key bash $TST/check_acceptance.sh` → 9/9 | 0 | ✅ |
