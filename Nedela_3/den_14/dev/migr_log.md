...
## Этап M6 — Тесты и отладка (2026-09-21 12:05)

- Статус: ✅ завершён
- Было: тестовый контур без тестов инвариантов.
- Стало: создан `dev/tests_debug/unit/test_invariants.py` (13 тестов); в `scenario.py` добавлен
  `scenario_invariant_conflict` (8-й сценарий); в `check_acceptance.sh` добавлены 2 проверки
  инвариантов (было 10 → стало **12**).
- Проверка (всё без живого ключа, временные файлы только в `.tmp/`):
  - `unit_runner.py` → **69 OK, 0 FAIL** (вкл. `test_invariants.py` 13/13);
  - `scenario.py` → **8 сценариев OK** (вкл. `scenario_invariant_conflict`);
  - `smoke.py` → **SMOKE OK**;
  - `check_acceptance.sh` → **12 из 12** (FAIL=0, EXIT 0).
- Артефакты: `dev/tests_debug/unit/test_invariants.py` (новый); `dev/tests_debug/scenario.py`;
  `dev/tests_debug/check_acceptance.sh`.
- Спорное/риски:
  - **R1 закрыт частично**: гейт фактически выполняет 12 проверок (не 11, как в эталоне) —
    счёт заявлен динамически; все зелёные. Точное число сверяется по факту.
  - **R2 закрыт**: сценариев теперь 8 (добавлен `scenario_invariant_conflict`) — совпадает с
    эталоном.
  - при отладке проверки 11 пойман баг экранирования (`>/dev/null` перед `| grep` съедал вывод;
    `"$MD"` внутри `bash -c "…"` не экранировался) — исправлено.
- Гейт M6→M7: ✅ пройден (L2 69 OK; L4 8 сценариев; гейт 12/12; `users/` проекта не тронут;
  прежние 25 критериев без регрессии).
- Перечитывание migr_plan.md перед следующим этапом: ✅ выполнено (2026-09-21 12:05)
- Следующий этап: `dev/migr_plan_7.md` (M7 — Документация)

---

## Этап M7 — Документация (2026-09-21 12:10)

- Статус: ✅ завершён
- Было: README без раздела инвариантов; `dev/Проверка.md` отсутствовал (R4 из M0).
- Стало: в `README.md` добавлен раздел «Инварианты и ограничения состояния (День 14)»
  (модель, хранение, промт, рабочий цикл, CLI-команды) + строки команд `/invariants`,
  `/invariant add|set|on|off`, `/check`; обновлены блоки «Тестирование» (69 OK / 8 сценариев /
  12 гейт) и «Ручная проверка» (30 критериев). Создан `dev/Проверка.md` — 30 критериев
  (19 прежних + 20–22 персонализация + 23–25 автомат + **26–30 инварианты**).
- Проверка: сверка формулировок README/Проверки с кодом (имена команд, полей, файлов,
  текст отказа) — расхождений нет; критерии 26–30 воспроизводимы по тексту (совпадают с
  `test_invariants.py` и `scenario_invariant_conflict`).
- Артефакты: `README.md`, `dev/Проверка.md` (новый).
- Спорное/риски: нет.
- Гейт M7→M8: ✅ пройден (README содержит раздел инвариантов и соответствует коду; Проверка
  содержит 26–30; критерии воспроизводимы).
- Перечитывание migr_plan.md перед следующим этапом: ✅ выполнено (2026-09-21 12:10)
- Следующий этап: `dev/migr_plan_8.md` (M8 — Финал: прогон и приёмка)

---

## Этап M8 — Финал: прогон и приёмка (2026-09-21 12:15)
...
- Проверка:
  ...
  - **L2** `unit_runner.py` → **69 OK, 0 FAIL**;
  - **L3** `smoke.py` → **SMOKE OK**;
  - **L4** `scenario.py` → **8 сценариев OK**;
  - **Гейт** `check_acceptance.sh` → **12 из 12** (FAIL=0, EXIT 0);
  - демонстрация: `/invariant add` → `/check перейти на FastAPI` → **ЗАПРЕЩЕНО**
    («Нарушен инвариант framework.django»); `invariants.json` создан на диске; лог
    `[Инварианты] …` присутствует;
  - артефакты §4.1 на месте: `core/invariants.py`, раздел «Инварианты» в `README.md`,
    `run.sh` (+x), `run.desktop`, `dev/Проверка.md` (30 критериев), `dev/migr_plan_0..8.md`.
- Артефакты: `dev/migr_log.md` (итог).
- Спорное/риски: **R1** (счёт гейта) — фактически 12 проверок (эталон заявлял 11; динамический
  счёт); **R2** — сценариев 8 (совпадает с эталоном); **R3** — git-репозитория нет (откат вручную);
  **R4** — `dev/Проверка.md` создан в M7 (закрыт).
- Гейт (финальный): ✅ пройден.
  - [x] целевые артефакты §4.1 созданы;
  - [x] критерии 26–30 зелёные;
  - [x] прежние 25 — без регрессии;
  - [x] L2→L3→L4→гейт (12/12) без живого ключа;
  - [x] этапы M0–M8 отражены в `migr_log.md` (✅), открытых ошибок нет;
  - [x] `migr_plan_0.md … migr_plan_8.md` созданы и исполнены;
  - [x] `migr_plan.md` перечитан после каждого этапа.
- Перечитывание migr_plan.md перед следующим этапом: — (финальный этап)
- Предложение коммита: сформировано (коммит — только по явной команде пользователя).

---

## Итог миграции den_14 → arch_den_14.md

- **Слой инвариантов** реализован аддитивно поверх наследия дней 11–13:
  `core/invariants.py` (`Invariant`/`ConstraintSet`/`ProposedAction`/`InvariantChecker`+
  `RuleBasedChecker`/`update_invariant`); `storage/store.py` (`invariants_path`/`read`/`write`);
  блок `[system: invariants]` в промте; проверка и отказ в `core/agent.py`; CLI-команды в `Kod.py`.
- **Критерии 26–30** зелёные; **прежние 25** без регрессии (всего **30/30**).
- **Тестовый контур**: L2 69 OK · L3 OK · L4 8 сценариев · гейт 12/12 — без живого ключа.
- **Файлы продукта изменены**: `core/invariants.py` (новый), `core/agent.py`,
  `core/prompt_builder.py`, `storage/store.py`, `Kod.py`, `README.md`.
- **Служебные**: `dev/Проверка.md` (новый), `dev/tests_debug/unit/test_invariants.py` (новый),
  `dev/tests_debug/scenario.py`, `dev/tests_debug/check_acceptance.sh`, `dev/migr_log.md`.
- **Наследие не тронуто**: `llm_client.py`, `memory/*`, `storage/db.py`, `state_machine.py`,
  `profile_router.py`.
...
❯ Миграция завершена. Итог по этапам M0–M8 (все гейты зелёные):


  ┌─────┬─────────────────────────────────────────────────────────────────────────────┬─────┐
  │ Эта │ Что сделано                                                                 │ Гей │
  ├─────┼─────────────────────────────────────────────────────────────────────────────┼─────┤
  │ M0  │ Базовая линия: L2 56 OK / L3 OK / L4 7 / гейт 10/10; расхождения и точки... │ ✅  │
  │ M1  │ core/invariants.py — модель + InvariantChecker/RuleBasedChecker + `updat... │ ✅  │
  │ M2  │ storage/store.py: invariants_path/read/write → invariants.json              │ ✅  │
  │ M3  │ Блок [system: invariants] в prompt_builder.py (+ правило роли)              │ ✅  │
  │ M4  │ Проверка/отказ в agent.py; _execute — единственный вызов инструмента; `T... │ ✅  │
  │ M5  │ CLI: /invariants, `/invariant add\                                          │ set │
  │ M6  │ test_invariants.py (13), scenario_invariant_conflict (8-й), гейт 10→12        │ ✅  │
  │ M7  │ Раздел «Инварианты» в README.md; dev/Проверка.md (30 критериев)             │ ✅  │
  │ M8  │ Финал: L1 ok · L2 69 OK · L3 OK · L4 8 · гейт 12/12; демонстрация отказа    │ ✅  │
  └─────┴─────────────────────────────────────────────────────────────────────────────┴─────┘


  Результат: агент работает в рамках заданных инвариантов — хранит их отдельно от диалога
  (invariants.json), явно учитывает в промте, программно блокирует конфликтующие действия с
  объяснением и альтернативой, изменение правила требует подтверждения. Критерии 26–30 + прежние 25
   — зелёные (30/30).

  Наследие дней 11–13 не тронуто (llm_client.py, memory/*, db.py, state_machine.py,
  profile_router.py).

  Изменены файлы продукта: core/invariants.py (новый), core/agent.py, core/prompt_builder.py,
  storage/store.py, Kod.py, README.md. Служебные: dev/Проверка.md,
  dev/tests_debug/unit/test_invariants.py (новые), scenario.py, check_acceptance.sh,
  dev/migr_log.md.
