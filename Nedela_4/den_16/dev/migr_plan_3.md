...
(`MCPClient` — делегирует транспорту, SDK не импортирует; handshake `initialize` +
`list_tools`; любой сбой → `MCPConnectionError`, не молчание).
...
**Предусловия:** `mcp` SDK импортируется **только** в `integrations/mcp/transport.py`
(ленивый импорт в `initialize`) и `demo_server.py` (M4); `client.py` делегирует
транспорту; `core/`/`memory/`/`storage/`/`Kod.py` слой не импортируют;
...
2. `DEFAULT_SERVERS` — конфиг демо-сервера по умолчанию: `server_id="demo"`,
   `transport="stdio"`, `command=(sys.executable, "-m",
...
2. `class StdioMCPTransport(MCPTransport)`: обёртка над `mcp` SDK — **ленивый
   импорт SDK внутри `initialize()`**; stdio-клиент (подпроцесс по `command` из
   конфига), `timeout_seconds` на вызовы; ошибки SDK → `MCPConnectionError`.
   Реализация — по фактическому API установленной версии SDK (`mcp.client.stdio` /
   `ClientSession`); при расхождении версий — зафиксировать фактический API в
   `migr_log.md`.
3. `class FakeMCPTransport(MCPTransport)`: детерминированная заглушка —
   ...
   `fail_list_tools`, `timeout`; метод `set_tools(...)` — смена каталога (для теста
   обновлений); `initialize`/`list_tools`/`close` — без сети и без подпроцесса.
4. `class MCPConnectionError(Exception)`: `server_id`, `reason`.
5. **Ожидаемый результат:** три транспорта + исключение; fake полностью
   детерминирован.

### Шаг 3.4 — `client.py`: MCPClient (делегирует транспорту)
...
4. Любой сбой (исключение транспорта/таймаут/битый формат) → `MCPConnectionError`
   (не `None`, не молчание).
5. **Ожидаемый результат:** клиент по arch §2.4; SDK напрямую не импортирует
   (транспорт инжектится; SDK — в `transport.py`).
...
# 7. import integrations.mcp.client — успешен (SDK импортируется лениво — в transport.initialize)
EOF
```
...
1. `grep -rn "import mcp\|from mcp" core/ memory/ storage/ Kod.py integrations/` →
   **только** `integrations/mcp/transport.py` (ленивый импорт; `demo_server.py` —
   M4); `client.py` — без прямого импорта SDK.
...
- [ ] SDK импортируется только в `integrations/mcp/transport.py` (ленивый импорт)
      + `demo_server.py` (grep);
