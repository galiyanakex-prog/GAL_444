...
1. `source ../../.venv/bin/activate && pip install mcp` (Model Context Protocol
   Python SDK; при конфликте зависимостей — зафиксировать в карточке ошибки).
2. `pip show mcp` → зафиксировать версию; `python -c "import mcp; print(mcp.__file__)"`.
