...
7. `class MCPGatewaySync`: тонкая синхронная обёртка (`asyncio.run`) над
   `start`/`stop`/`discover`/`status` — для REPL и `--mcp-probe` (REPL остаётся
   синхронным; полный async core — задел).
...
```bash
python - <<'EOF'
# StdioMCPTransport(command=("python","-m","integrations.mcp.demo_server"))
...
2. `grep -rn "import mcp\|from mcp" integrations/` → только `client.py` и
   `demo_server.py`.
...
`read/write_mcp_servers`, `tool_catalog_path`/`save/load_tool_catalog`, задел
