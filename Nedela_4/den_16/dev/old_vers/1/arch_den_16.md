...
- **Хранение — только через фасад `Store`**: `MCPGateway` не пишет JSON напрямую;
  новые методы фасада `save_tool_catalog` / `load_tool_catalog` (+ задел
  `append_tool_audit`); каталог серверов — user-scope, история вызовов (задел) —
  task-scope.
- **Асинхронность внутри, синхронный фасад наружу**: транспорт/gateway/provider —
  ...
  `Kod.py` остаётся синхронным и зовёт gateway через тонкую синхронную обёртку
  (`asyncio.run`). Полный async core (вариант A из рекомендаций) — задел следующих
  дней; для минимального дня 16 выбран вариант B («async gateway за синхронным
  фасадом») — меньше регрессионного риска при том же контракте.
...
├── core/
│   ├── __init__.py
│   ├── agent.py                 # оркестратор (без изменений; + тонкая интеграция /mcp-отчётов)
...
│       ├── transport.py         # MCPTransport (ABC) + StdioMCPTransport (mcp SDK, stdio) +
│       │                        #   FakeMCPTransport (детерминированная заглушка для тестов)
│       ├── client.py            # MCPClient: initialize / list_tools / (задел: call_tool) —
│       │                        #   единственное место, где живёт mcp SDK
...
│   │                            #   save_tool_catalog/load_tool_catalog (миграция: нет файла → пустой
│   │                            #   каталог schema_version=1; задел: append_tool_audit)
...
минимальное касание (отчётные методы, без изменения контрактов жизненного цикла);
...
`call_tool`-задел / `close`) + `StdioMCPTransport` (обёртка над `mcp` SDK:
stdio-подпроцесс, timeout) + `FakeMCPTransport` (детерминированная заглушка:
фиксированный список тулов, программируемые отказы/таймауты/`is_error`, смена
каталога — для тестов без сети и без SDK-подпроцесса).

**Клиент** (`client.py`) — `MCPClient`: единственное место импорта `mcp` SDK;
...
Синхронный фасад для REPL: `MCPGatewaySync` (тонкая обёртка `asyncio.run` над
start/stop/discover/status) — REPL и `--mcp-probe` остаются синхронными.
...
- `Store.mcp_servers_path/read_mcp_servers/write_mcp_servers`;
- `Store.tool_catalog_path/save_tool_catalog/load_tool_catalog` — миграция:
  отсутствующий/битый файл → пустой каталог `{"schema_version": 1, "version": 0,
  "tools": [], "created_at": null}`; каталог загружается при старте (если MCP
  включён) и пересохраняется после каждого успешного `refresh()`;
...
```python
mcp_enabled = False  # по умолчанию; включается флагом --mcp или командой /mcp connect

if config.mcp_enabled:
    gateway = MCPGateway(config.mcp)          # integrations/
    provider = MCPToolProvider(gateway)
    tool_registry.add_provider(provider)
...
| `/mcp disconnect <id>` | закрыть соединение (тулы сервера покидают каталог при следующем refresh) |
...
1. **Хранилище** (`storage/`) — прежний фасад `Store` + новые методы:
   ...
   каталога); битый файл → дефолт, приложение не падает.
...
4. **Интеграции (integrations/mcp/)** — «как достучаться до внешнего мира».
   ...
   - transport: ABC + StdioMCPTransport (mcp SDK, подпроцесс, timeout) +
     FakeMCPTransport (программируемые списки/отказы/смена каталога — тесты без
     сети).
   ...
   - gateway: start/stop/status/discover; MCPConnectionState
     ...
     Синхронный фасад MCPGatewaySync (asyncio.run) для REPL.
...
5. **Оркестратор и CLI** — «как этим пользуются». Agent дней 11–15 без изменений
   (жизненный цикл: /plan → /approve → /step|/run → validation → done; инварианты;
   профили; память). DI: build_agent() опционально строит gateway+provider и
   регистрирует в ToolRegistry только при mcp_enabled (по умолчанию False).
   Команды: /mcp status | servers | tools | refresh | connect <id> |
   disconnect <id>; флаги: --mcp (включить слой), --mcp-probe (one-shot:
   подключиться → вывести список тулов → закрыться). /mcp-команды токенов LLM не
   тратят.
