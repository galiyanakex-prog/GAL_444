...
2. При `mcp_enabled=True`:
   ...
   `registry.refresh()` / `store.save_tool_catalog(...)` при сборке **не**
   вызываются (discovery + сейв каталога — только по явной команде `/mcp refresh`).
...
6. `/mcp disconnect` — закрыть **все** соединения (`gateway.stop()` → DISCONNECTED;
   тулы покидают каталог при следующем refresh).
...
2. `--mcp-probe` — one-shot результат задания: `load_servers_config(...)` →
