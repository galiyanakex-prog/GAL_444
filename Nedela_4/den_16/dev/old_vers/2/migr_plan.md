...
> изменялся**; исходная версия — в `dev/old_vers/`.
...
3. **MCP-слой** — `integrations/mcp/`: `config.py` (`MCPServerConfig` + загрузка
   ...
   `mcp` SDK — ленивый импорт SDK в `initialize()`; + `FakeMCPTransport`),
   `client.py` (`MCPClient`: initialize / list_tools — делегирует транспорту,
   SDK не импортирует), `gateway.py` (`MCPGateway` + `MCPConnectionState` +
   синхронный фасад `MCPGatewaySync` на постоянном фоновом loop), `provider.py`
...
- **Инварианты самой миграции**: (1) не менять контракты дней 11–15 (вне новых
  ...
  протекает в `core` (SDK импортируется только в `integrations/mcp/transport.py`
  (ленивый импорт) + `demo_server.py`; `client.py` делегирует транспорту);
...
- **Шаги**:
  1. Установить `mcp` (Model Context Protocol Python SDK) в venv недели
     (`../../.venv/bin/python -m pip install mcp` — вызов pip-скрипта напрямую
     ненадёжен из-за shebang копированного venv); зафиксировать версию; проверить
...
- **Шаги**:
  ...
  3. `transport.py`: `MCPTransport` (ABC: initialize / list_tools / call_tool-задел /
     close) + `StdioMCPTransport` (обёртка над `mcp` SDK: ленивый импорт SDK в
     `initialize()`, stdio-подпроцесс, timeout) + `FakeMCPTransport`
  ...
  4. `client.py`: `MCPClient` — транспорт инжектится, SDK напрямую не импортирует
     (SDK — в `transport.py`); `initialize()` (handshake: protocol version,
     capabilities), `list_tools()` → нормализованные сырые описания
     `{name, description, inputSchema}`; любой сбой → `MCPConnectionError`
     (не `None`, не молчание).
...
- **Гейт M3→M4**: модули импортируются; смоук зелёный (дефолты, битый файл,
  ...
  только в `transport.py` (ленивый импорт) + `demo_server.py` (grep-проверка).
...
- **Шаги**:
  1. `gateway.py`: `MCPConnectionState` (disconnected/connecting/ready/degraded/
     ...
     НЕ знает о TaskStage/профилях/памяти) + `MCPGatewaySync` (синхронный фасад на
     постоянном фоновом event loop: daemon-поток + `run_coroutine_threadsafe`;
     сессии `mcp` SDK привязаны к loop'у initialize, `asyncio.run` на каждый вызов
     несовместим).
...
- **Шаги**:
  1. `storage/store.py`: `mcp_servers_path` / `read_mcp_servers` /
     `write_mcp_servers` (отсутствующий/битый файл → `None`; дефолт
     `[DEFAULT_SERVERS]` — у вызывающего через `load_servers_config`);
     `tool_catalog_path` / `read_tool_catalog` / `save_tool_catalog(user_id, data:
     dict)` (`read_tool_catalog` при отсутствии/битости → `None`; дефолт пустого
     каталога `{"schema_version": 1, "version": 0, "tools": [], "created_at": null}`
     — у вызывающего; `save_tool_catalog` принимает готовый dict).
  2. `append_tool_audit` / `load_tool_audit` — `users/<id>/tasks/<task>/
     tool_audit.jsonl` (append-only, битые строки пропускаются) — **реализованы
     полностью**; рабочий вызов — день 17+.
...
- **Шаги**:
  1. `build_agent()`: опциональная сборка MCP-слоя при `mcp_enabled`
     (`gateway = MCPGatewaySync(load_servers_config(...))` → `MCPToolProvider` →
     `tool_registry.add_provider`; `agent.mcp_gateway` / `agent.tool_registry`);
     `refresh()` / `save_tool_catalog` при сборке **не** вызываются (только по
     `/mcp refresh`); по умолчанию `mcp_enabled=False` (оба атрибута агента — None).
  2. Семейство `/mcp` (6 форм): `status` (состояние подключения + версия каталога),
     ...
     refresh → атомарный swap → сейв каталога), `connect <id>`, `disconnect` (закрывает **все** соединения).
...
- **Модель MCP не протекает в `core`**: `mcp` SDK импортируется только в
  `integrations/mcp/transport.py` (ленивый импорт) + `demo_server.py`
  (`client.py` делегирует транспорту); `ToolDescriptor` — внутренняя модель агента.
...
| SDK протёк в ядро | `core/` импортирует `mcp` | grep-проверка на каждом этапе; SDK только в `integrations/mcp/transport.py` (+ `demo_server.py`) |
